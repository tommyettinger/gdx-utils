...
/** modified version of {@link com.badlogic.gdx.maps.tiled.TmxMapLoader TmxMapLoader} that loads {@link TextureMapObject TextureMapObjects} from Tiled's image objects
...
public class TmxMapLoader extends AsynchronousAssetLoader<TiledMap, TmxMapLoader.Parameters> {
	...
	protected void loadObject(MapLayer layer, Element element) {
		if(element.getName().equals("object")) {
			...
			if(gid != -1) {
				object = new TextureMapObject(map.getTileSets().getTile(gid).getTextureRegion());
				object.getProperties().put("gid", gid);
				TextureMapObject texObject = (TextureMapObject) object;
				texObject.setX(x);
				texObject.setY(y);
			}
